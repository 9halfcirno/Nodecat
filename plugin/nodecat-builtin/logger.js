...
const p = {
	...
	main(cat) {
		cat.onMessage.all().then(async msg => {
			...
			let msgNum = cat.data.get("get_msg_num") || 0;
			cat.data.set("get_msg_num", ++msgNum);
			...
			// 消息内容
			output += `: \n${BOLD}${WHITE}${await msg.getDetailedString() || ""}${RESET}`;
		...
		cat.onMessageSent.then(async msg => {
			...
			let msgNum = cat.data.get("send_msg_num") || 0;
			cat.data.set("send_msg_num", ++msgNum);
			...
			// 消息内容
			output += `${BOLD}${YELLOW}${await msg.getDetailedString() || ""}${RESET}`;
