...
import NormalAPI from "./normal_api.js"
...
class PluginContext {
	...
	async unload() {
		...
		try {
			await this.plugin.unload?.(this.api);
		} catch(e) {
			console.error(`[Plugin] 插件 ${this.id} unload函数执行错误`, e)
		}
	...
	triggerNotice(data) {
		this.api && this.api.noticeTriggers.forEach(t => t(data))
	}
